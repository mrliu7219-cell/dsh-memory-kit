...
### v0.1.2 — 踩坑库模板升级（完成）
- 对标 MisakaNet：traps 模板加 **Lesson ≠ Skill 纪律**（只记失败经验不写教程，防库跑偏）
- 明确 **✅ 已验证 = 该坑防错已实测**（补齐"验证"环节的措辞；原有三档已覆盖，无需改结构）
- 加可选 **证据等级标注**（亲历/查证/推断）——帮维护时判断可信度，呼应写侧治理
- 同步：README（踩坑库能力描述）、setup.js（MEMORY.md/traps.md 模板正文）、本日志
- 不影响已有坑数据；仅模板措辞与纪律注释

### v0.1.3 — 代码复查修复（完成）
- **修复** `notes-semantic-search.sh` 的 `-n` 参数解析 bug（查询在前时 `-n` 被拼进查询串；改为从参数中剔除，位置无关，bash 3.2 实测三种位置全过）
- **加固** `lib/setup.js` mergeHooks 幂等：改为按 hook 命令 basename 去重（同事件已有同名脚本不重复注入；原字符串包含判断有 &&/|| 优先级隐患）
