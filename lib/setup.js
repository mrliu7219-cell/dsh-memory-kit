...
function initMemoryDir() {
  ...
  if (!fs.existsSync(traps)) {
    fs.writeFileSync(traps, `# 踩坑库（traps.md）— 所有犯过的错，教训 + 防错规则\n条目格式：现象 → 根因 → 防错 → 状态。\n**状态三档**：🆕 新鲜（刚踩）/ ✅ 已验证（防错起效）/ ⬆️ 已升级（同类犯 2 次，规则进 AGENTS.md）\n\n---\n`);
    ok(`踩坑库模板已建: ${traps}`);
  }
}
